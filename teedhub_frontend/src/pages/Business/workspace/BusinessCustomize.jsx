...
import { Upload, AlertCircle, CheckCircle, Palette } from 'lucide-react';
import { useToast } from '@/hooks/useToast';
import { apiGet, apiPost } from '@/utils/api';
import { API_BASE_URL } from '@/utils/constants';

export default function BusinessCustomize() {
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-300 mt-1">
                Only owner and admin members can customize business branding.
              </p>
            </div>
          </div>
        </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        success('Business customization updated successfully!');
        setTimeout(() => {
          navigate(`/business/${business.id}/overview`);
        }, 1500);
      } else {
        const errorData = await response.json();
        errorToast(errorData.error || 'Failed to update customization.');
      }
    } catch (err) {
